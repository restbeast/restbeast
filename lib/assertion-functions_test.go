...
func Test_assertUUIDv4(t *testing.T) {
	tests := []struct {
		...
		{
			"invalid UUIDv4", []cty.Value{cty.StringVal("i-am-not-a-valid-uuid")},
			cty.StringVal(`expected i-am-not-a-valid-uuid to be a valid UUIDv4`),
		},
	}
...
func Test_assertIpv4(t *testing.T) {
	tests := []struct {
		...
		{
			"invalid ipv4", []cty.Value{cty.StringVal("i-am-not-a-valid-ip")},
			cty.StringVal(`expected i-am-not-a-valid-ip to be a valid ipv4`),
		},
	}
...
func Test_assertRegex(t *testing.T) {
	tests := []struct {
		...
		{
			"mismatch", []cty.Value{cty.StringVal("^[a-z]+$"), cty.StringVal("42")},
			cty.StringVal("expected 42 to be a valid ^[a-z]+$"),
		},
	}
...
func Test_assertEqual(t *testing.T) {
	tests := []struct {
		...
		{
			"not equal values", []cty.Value{cty.StringVal("valueA"), cty.StringVal("valueB")}, cty.StringVal(
				`expected: "valueA"
got: "valueB"`,
			),
		},
...
func Test_assertNotEqual(t *testing.T) {
	tests := []struct {
		...
		{
			"equal values", []cty.Value{cty.StringVal("valueA"), cty.StringVal("valueA")}, cty.StringVal(
				`not expected: "valueA"
got: "valueA"`,
			),
		},
...
func Test_assertGreaterThan(t *testing.T) {
	tests := []struct {
		...
		{
			"not greater value", []cty.Value{cty.NumberIntVal(int64(1)), cty.NumberIntVal(int64(2))},
			cty.StringVal(`expected 1 to be greater than 2`),
		},
	}
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got, _ := assertionFunctionList["assertGreaterThan"].Impl(
					tt.args, cty.String,
				); tt.want.NotEqual(got).True() {
...
func Test_assertGreaterThanOrEqualTo(t *testing.T) {
	tests := []struct {
		...
		{
			"not greater value", []cty.Value{cty.NumberIntVal(int64(1)), cty.NumberIntVal(int64(2))},
			cty.StringVal(`expected 1 to be greater than or equal to 2`),
		},
	}
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got, _ := assertionFunctionList["assertGreaterThanOrEqualTo"].Impl(
					tt.args, cty.String,
				); tt.want.NotEqual(got).True() {
...
func Test_assertLessThan(t *testing.T) {
	tests := []struct {
		...
		{
			"not greater value", []cty.Value{cty.NumberIntVal(int64(2)), cty.NumberIntVal(int64(1))},
			cty.StringVal(`expected 2 to be less than 1`),
		},
	}
...
func Test_assertLessThanOrEqualTo(t *testing.T) {
	tests := []struct {
		...
		{
			"not greater value", []cty.Value{cty.NumberIntVal(int64(2)), cty.NumberIntVal(int64(1))},
			cty.StringVal(`expected 2 to be less than or equal to 1`),
		},
	}
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got, _ := assertionFunctionList["assertLessThanOrEqualTo"].Impl(
					tt.args, cty.String,
				); tt.want.NotEqual(got).True() {
...
func Test_assertNonEmptyString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(
			tt.name, func(t *testing.T) {
				if got, _ := assertionFunctionList["assertNonEmptyString"].Impl(
					tt.args, cty.String,
				); tt.want.NotEqual(got).True() {
					t.Errorf("assertFalse() = %v, want %v", got.AsString(), tt.want.AsString())
				}
